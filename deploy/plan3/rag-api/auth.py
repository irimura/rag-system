...
def derive_group_password(group: str) -> str:
    """グループ別 internal user の再現可能なパスワードを導出する。"""
    import hashlib
    import hmac

    secret = os.environ.get("OS_GROUP_USER_SECRET", "")
    if not secret:
        raise RuntimeError("OS_GROUP_USER_SECRET が未設定です")
    return hmac.new(secret.encode(), ("rag_" + group).encode(), hashlib.sha256).hexdigest()
