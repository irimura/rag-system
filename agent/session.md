...
- 実行環境で AES 暗号化 PDF のページ数検証に `cryptography` が必要と判明したため、`cryptography==49.0.0` を固定依存へ追加した。依存追加前に取得済みの PDF は、venv 更新後の再実行でダウンロードをスキップして再検証できる

## 24. Allganize 日本語 PDF 変換比較資材(2026-08-06)

- `04-corpus/allganize-ja/` に9プロダクトの説明・変換・比較手順、層化サンプル選定、共通メトリクス付き変換ラッパー、集約スクリプト、点検記録を配置した
- サンプル約10件を先に比較し、精度・表再現性・成功率・リソース・ライセンス条件を満たす用途別上位2製品までを64件全件へ進める2段階構成とした
- AnyDocは文字埋込みPDF専用、YomiTokuはCC BY-NC-SA 4.0で商用不可、MinerUの現行配布物はAGPL-3.0、Markerはコードとモデル重みの条件が異なるため警告を明記した
- 変換済みMarkdownは既定でスキップし、1文書ごとにメトリクスCSVを更新する。RAM 16 GBを考慮して並列度1とし、GPU系はL4を既定にした
- ローカルにはデータセットとPDFが未配置のため、実サンプル選定、各製品のインストール・モデル取得・実変換・精度実測は未実施
- レビュー対応で、olmOCR既定起動を `python -m olmocr.pipeline` へ修正し、RAM/VRAMを変換プロセスのPID系統だけから0.5秒間隔で計測するよう変更した。失敗時のページ/秒は空欄とする
- 文書タイプはURL等から推測せず、明示メタデータがない場合は未判定とする。CSV列名を `document_type_guess` とし、全行を原本で確認する運用にした
