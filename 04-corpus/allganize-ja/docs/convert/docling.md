# Docling 標準パイプライン変換手順書

## 概要

コードはMITです。利用するOCR・解析モデルのモデルカードも導入時に保存してください。

標準パイプラインでPDFバックエンド、レイアウト解析、OCR、表認識を使います。CPUでも動きますが、利用可能ならCUDAを使います。
...
python -m pip install 'docling==2.*' pypdf psutil
python -m pip freeze > metrics/docling-versions.txt
```
...
出力は `out/docling/<元ファイル名>.md`、メトリクスは `metrics/docling.csv` です。既存の空でないMarkdownはスキップします。
...
```bash
source .venv-docling/bin/activate
.venv-docling/bin/python scripts/convert_docling.py --all
```
...
```bash
column -s, -t metrics/docling.csv | less -S
...
- 日本語OCRが空になる場合は、DoclingのOCRエンジンと言語設定を版のCLIヘルプで確認します。
- RAM不足時はほかのプロセスを止めます。
- 初回モデル取得後に再実行して速度を測ります。

CLIが導入版と異なる場合は、既定値を直接編集せず、まず `.venv-docling/bin/python scripts/convert_docling.py --command '{input} を含むコマンド'` で1件を試します。動作確認後に版と変更理由を文書へ反映します。
